...
export async function POST(req: Request) {
  ...
  if (lockout.locked) {
    audit({ ip, action: "login.blocked", meta: { remainingSeconds: lockout.remainingSeconds } });
    return NextResponse.json(
      { error: `Too many failed attempts. Try again in ${Math.ceil(lockout.remainingSeconds / 60)} minute(s).`, locked: true, remainingSeconds: lockout.remainingSeconds },
      { status: 429 }
    );
  }
  ...
  if (!checkPassword(password)) {
    ...
    if (next.locked) {
      return NextResponse.json(
        { error: `Too many failed attempts. Locked for ${Math.ceil(next.remainingSeconds / 60)} minute(s).`, locked: true, remainingSeconds: next.remainingSeconds },
        { status: 429 }
      );
    }
  ...
  cookies().set("ha_session", sessionId, { httpOnly: true, secure: isProd, sameSite: "strict", path: "/", maxAge: 4 * 60 * 60 });
